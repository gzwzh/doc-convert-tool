...
### Q2: 权限不足怎么办？
**答**：如果安装目录在 `C:\Program Files`，Updater 需要管理员权限。
- **方案 A**：在 manifest 中声明 `requireAdministrator`，启动时会弹出 UAC 提示。
- **方案 B**：安装到用户目录（`AppData/Local/`），无需提权（推荐）。

### Q3: 更新失败如何回滚？
**答**：
1. 在解压前，将当前目录重命名为 `backup_v1.0.0`。
2. 解压新版本。
3. 如果解压失败或校验失败，删除新文件，将 backup 目录还原。

### Q4: 如何调试 Updater？
**答**：Updater 通常是静默运行的，建议开启文件日志记录（如 `updater_log.txt`），将每一步的操作结果写入日志以便排查。
